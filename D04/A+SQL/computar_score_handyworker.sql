...
CREATE PROCEDURE `acme-handyworker`.`computar_score_handyworker` (IN handyworker_id INT)
BEGIN
	...
	DECLARE comments CURSOR FOR SELECT ENDORSEMENT.COMMENTS FROM ENDORSEMENT WHERE ENDORSEMENT.RECIPIENT = handyworker_id; # A LA VARIABLE COMMENTS LE ASIGNOS LOS COMENTARIOS DE LOS ENDORSEMENTS QUE TIENE EL HANDYWORKER COMO RECIPIENT
	...
	UPDATE HANDYWOKER SET SCORE = @computed_score WHERE id = handyworker_id; 
	COMMIT WORK;
